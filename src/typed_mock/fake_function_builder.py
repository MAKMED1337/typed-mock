...
class FakeFunctionBuilder[**P, R]:
    def __init__(self, fake_method: FakeFunction[P, R | Awaitable[R]]) -> None:
        self.__fake_method = fake_method

    @overload
    def return_(self, /, *values: R) -> 'FakeFunctionBuilder[P, R]': ...
    @overload
    def return_(self, value: R, /, *, times: int = 1) -> 'FakeFunctionBuilder[P, R]': ...

    def return_(self, /, *values: R, times: int = 1) -> 'FakeFunctionBuilder[P, R]':
        if not values:
    ...
    def raise_[E: BaseException](self, error: E | type[E], *, times: int = 1) -> 'FakeFunctionBuilder[P, R]':
        self.__fake_method.add_producer(raise_impl(error, times=times))
        return self

    def called_with(self, *args: object, return_: R, **kwargs: object) -> 'FakeFunctionBuilder[P, R]':
        return self.called_with_partial(*args, return_=return_, **kwargs)

    def called_with_partial(self, *args: object, return_: R, **kwargs: object) -> 'FakeFunctionBuilder[P, R]':
        self.__fake_method.add_producer(called_with_partial_impl(args, kwargs, return_, self.__fake_method.signature))
        return self

    def called_with_full(self, args: Args[P], return_: R) -> 'FakeFunctionBuilder[P, R]':
